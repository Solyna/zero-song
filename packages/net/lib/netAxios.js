...
import { cloneDeep } from '@zero-song/utils';
/**
 * 正在进行中的请求
 */
const pending = {};
...
class Net {
    ...
    constructor() {
        ...
        this.instance.interceptors.request.use(this.requestHandler, this.requestErrorHandler);
        this.instance.interceptors.response.use(this.responseHandler, this.responseErrorHandler);
        this.interceptors = {
    ...
    requestHandler = async (config) => {
        ...
        for (let interceptor of this.interceptors.request.handlers) {
            config = await promise.then(interceptor.fulfilled, interceptor.fulfilled);
        }
        /**
         * 赋值取消请求方法
         */
    ...
    responseHandler = async (resp) => {
        ...
        for (let interceptor of this.interceptors.response.handlers) {
            try {
                newData = await promise.then(interceptor.fulfilled, interceptor.rejected);
            }
            catch (error) {
                errorData = error;
            }
    ...
    responseErrorHandler = (resp) => {
        ...
        return Promise.reject({
            msg: resp.message || '未可知错误，大部分是由于后端不支持CORS或无效配置引起',
            code: status || 403,
        });
    };
}
